...
[t, x] = solvesystem_wangq323(x1, x2, t0, tN, x0, h);

% Exact Solution
% x1(t) = exp(-t/4).*(3 / 20 * cos(sqrt(151)*t/4) - sqrt(151) / 20 * sin(sqrt(151)*t/4) + 17 / sqrt(151) * (sqrt(151) / 20 * cos(sqrt(151)*t/4) + 3 / 20 * sin(sqrt(151)*t/4)));
% x2(t) = exp(-t/4).*(cos(sqrt(151)*t/4) + 17 / sqrt(151) * sin(sqrt(151)*t/4));

x1_sol = exp(-t/4).*(3 / 20 * cos(sqrt(151)*t/4) - sqrt(151) / 20 * sin(sqrt(151)*t/4) + 17 / sqrt(151) * (sqrt(151) / 20 * cos(sqrt(151)*t/4) + 3 / 20 * sin(sqrt(151)*t/4)));
x2_sol = exp(-t/4).*(cos(sqrt(151)*t/4) + 17 / sqrt(151) * sin(sqrt(151)*t/4));

% Plot
plot(x(1,:), x(2,:), '.-r', x1_sol, x2_sol, 'b');
ylabel('x2');
xlabel('x1');
title('Exercise 2 Plot');
legend("IEM", "Exact Solution", 'Location', 'best');
%% Exercise 3
...
% Plot
plot(x_EM(1,:), x_EM(2,:), 'b',x(1,:), x(2,:), '.-r', x1_sol, x2_sol, 'g');
xlabel('x1');
ylabel('x2');
title("Exercise 3 Plot");
legend("EM", "IEM", 'Exact Solution','Location','Best');

% The EM provides a extremely inaccurate estimation when compared to the
% IEM and the exact solution. It appears that the EM approx. think the plot
% is spiralling inward at a much lower rate compared to the exact solution
% and the IEM. 
